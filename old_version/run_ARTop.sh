...
anaconda3-launch python $script_path/source/python/numbering.py ${array[2]} ${array[3]} ${array[4]} ${array[5]} ${array[6]} ${array[7]} ${array[8]} ${array[9]} $outputdir $inputdir ${script_path} ${array[0]}

# regionData if no download
if [ "${array[1]}" == "false" ]; then
    > $outputdir/regionData.dat
    anaconda3-launch python $script_path/source/python/buildRegDat.py $inputdir $outputdir ${array[0]} ${array[2]} ${array[3]} ${array[4]} ${array[5]}
fi
...
echo '  '
if [ "${array[17]}" == "true" ]; then
echo 'Calculating the velocity distributions..'
# calculate the velocity distributions
anaconda3-launch python $script_path/source/python/DAVE4vm.py ${array[0]} ${array[2]} ${array[3]} ${array[4]} ${array[5]} ${array[6]} ${array[7]} ${array[8]} ${array[9]} $inputdir $outputdir ${array[10]} $endfl
fi
...
if [ "${array[18]}" == "true" ]; then
#anaconda3-launch python $script_path/source/python/potentialbxby.py ${array[0]} 0 $endfl $nx $ny $outputdir
#anaconda3-launch python $script_path/source/python/green_fn_potentialbxby.py ${array[0]} 0 $endfl $nx $ny $outputdir
cd $src/c++/
chmod u+x compPotCode
./compPotCode
./potentialfiled $nx $ny 0 $endfl ${array[0]} $outputdir
fi
##----------------------------------------------------------------------------
...
#	./ARTop $outputdir/Ux_${array[0]}_${array[10]}_$i.txt $outputdir/Uy_${array[0]}_${array[10]}_$i.txt $outputdir/Uz_${array[0]}_${array[10]}_$i.txt $outputdir/bx_${array[0]}_$i.txt $outputdir/by_${array[0]}_$i.txt $outputdir/bz_$#{array[0]}_$i.txt $nx $ny 360 360 ${array[14]} $outputdir/Bxp_${array[0]}_$i"Gf".txt $outputdir/Byp_${array[0]}_$i"Gf".txt $outputdir/windDatPotentialFastCO${array[14]}_VS${array[10]}_${array[15]}_$i.dat ${array[15]} >&2
